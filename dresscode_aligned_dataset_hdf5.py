...
class AlignedDatasetHDF5(BaseDataset):
    def initialize(self, opt, mode='train', stage='warp'):
        ...
        save_path = '/workspace/DressCode_train/DressCode_1024/dresscode.hdf5'
        hf = h5py.File(save_path, 'r')
        self.hf = hf

        if mode == 'train':
            self.mode = 'train'
            kk = np.array(hf['annots']['train'])
        else:
            self.mode = 'test'
            kk = np.array(hf['annots']['test_paired'])
        ...
        for line in lines:
            ...
            P_path = (c_type, 'image', p_name)
            # C_path = os.path.join(self.root, c_type, 'cloth_align', c_name)
            C_path = (c_type, 'cloth_align', c_name)
    ...
    def __getitem__(self, index):
        ...
        i_path, j_path, k_path = self.P_paths[index]
        kk_path = np.array(self.hf[i_path][j_path][k_path])
        P = Image.open(io.BytesIO(kk_path)).convert('RGB')
        P_np = np.array(P)
        ...
        kk_path = np.array(self.hf[i_path]['pose_25'][k_path+'.npy'])
        pose_data = np.load(io.BytesIO(kk_path))[0]
        ...
        kk_path = np.array(self.hf[i_path]['skeletons'][k_path[:-6]+'_5.jpg'])
        openpose_img = Image.open(io.BytesIO(kk_path)).convert('RGB')
        ...
        kk_path = np.array(self.hf[i_path]['densepose'][k_path[:-4]+'.png'])
        dense_mask = Image.open(io.BytesIO(kk_path)).convert('L')
        transform_for_mask = get_transform(self.opt, params, method=Image.NEAREST, normalize=False)
        ...
        kk_path = np.array(self.hf[i_path]['parse-bytedance'][k_path[:-4]+'.png'])
        parsing = Image.open(io.BytesIO(kk_path)).convert('L')
        ...
        if C_type == 'upper':
            if self.ratio_dict is None:
                ...
                pc_ratio = self.ratio_dict[self.C_paths[index][2][:-4]+'.png']
                if pc_ratio < 0.95:
                    preserve_mask_for_loss_np = np.array([(parsing_np==index).astype(int) for index in [1,2,3,4,7,8,9,10,12,13,14,17,18,19,20,23,26,27,28]])
                    preserve_mask_for_loss_np = np.sum(preserve_mask_for_loss_np,axis=0)
                elif pc_ratio < 1.0:
        ...
        i_path, j_path, k_path = self.C_paths[index]
        kk_path = np.array(self.hf[i_path][j_path][k_path])
        C = Image.open(io.BytesIO(kk_path)).convert('RGB')
        C_tensor = transform_for_rgb(C)

        # CM_path = C_path.replace('/cloth_align/', '/cloth_align_mask-bytedance/')
        # CM = Image.open(CM_path).convert('L')
        kk_path = np.array(self.hf[i_path]['cloth_align_mask-bytedance'][k_path])
        CM = Image.open(io.BytesIO(kk_path)).convert('L')
        ...
        kk_path = np.array(self.hf[i_path]['cloth_align_parse-bytedance'][k_path])
        cloth_parsing = Image.open(io.BytesIO(kk_path)).convert('L')
